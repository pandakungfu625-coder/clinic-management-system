...
> Note: This repository has been extended into a minimal **Clinic Management System** (branch `feature/clinic-mvp`). New features include **Patients**, **Doctors**, and **Appointments** with backend endpoints and frontend pages (`/patients`, `/doctors`, `/appointments`). The database file used for the clinic is `clinic.db`.
